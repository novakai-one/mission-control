...
const clamp = (value: number, lower: number, high: number): number => Math.min(high, Math.max(lower, value));
...
export function StudioResizeSeams() {
  ...
  useEffect(() => {
    const appEl = document.querySelector<HTMLElement>('.studio-app');
    appRef.current = appEl;
    if (!appEl) return;
    const { railPx, aiPct } = widthsRef.current;
    if (railPx) appEl.style.setProperty('--st-rail-w', `${railPx}px`);
    if (aiPct) appEl.style.setProperty('--st-ai-w', `${aiPct}%`);
  }, []);
