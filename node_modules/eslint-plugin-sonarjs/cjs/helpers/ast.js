...
exports.isOnEnclosingTemplateDelimiterLine = isOnEnclosingTemplateDelimiterLine;
...
exports.hasStringFirstArgument = hasStringFirstArgument;
 ...
 * template literal.
 */
function isOnEnclosingTemplateDelimiterLine(node) {
    if (!node.loc) {
        return false;
    }
    const enclosingTemplate = (0, ancestor_js_1.findFirstMatchingAncestor)(node, ancestor => ancestor.type === 'TemplateLiteral');
    return (node.loc.start.line === enclosingTemplate?.loc?.start.line ||
...
function hasStringFirstArgument(node) {
