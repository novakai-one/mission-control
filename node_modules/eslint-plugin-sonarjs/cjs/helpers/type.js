...
exports.isAny = isAny;
exports.areMutuallyAssignableTypes = areMutuallyAssignableTypes;
exports.areSameTypeDeclarations = areSameTypeDeclarations;
 ...
 */
function areMutuallyAssignableTypes(checker, left, right) {
 ...
 */
function areSameTypeDeclarations(checker, left, right) {
    if (!left || !right || isAnyOrUnknownType(left) || isAnyOrUnknownType(right)) {
        return false;
    }
...
function areSameTypeArguments(checker, left, right) {
    if (!left || !right) {
        return false;
    }
    if (isAnyOrUnknownType(left) || isAnyOrUnknownType(right)) {
        return (isAnyOrUnknownType(left) &&
...
function isAnyOrUnknownType(type) {
    return (type.flags & (typescript_1.default.TypeFlags.Any | typescript_1.default.TypeFlags.Unknown)) !== 0;
}
/**
 ...
 */
function getComparableDeclaredTypeSymbol(type) {
    const symbol = type.aliasSymbol ?? type.symbol;
    return symbol && (symbol.flags & typescript_1.default.SymbolFlags.TypeLiteral) === 0 ? symbol : undefined;
}
function getDeclaredTypeArguments(type, checker) {
...
function isTypeReference(type) {
    return ((type.flags & typescript_1.default.TypeFlags.Object) !== 0 &&
        (type.objectFlags & typescript_1.default.ObjectFlags.Reference) !== 0);
}
/**
 ...
 * @param services The services used to get access to the TypeScript type checker
 */
function isBooleanArray(type, services) {
...
function isBooleanType({ flags }) {
    return (flags & typescript_1.default.TypeFlags.BooleanLike) !== 0;
}
/**
